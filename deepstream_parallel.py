...
def main(args, requested_pgie=None, config=None, disable_probe=False):
    ...
    # print("Creating metamux \n ")
    
    # # Create nvstreammux instance to form batches from one or more sources.
    # meta_muxer = Gst.ElementFactory.make("nvdsmetamux", "meta-muxer")
    # if not meta_muxer:
    #     sys.stderr.write(" Unable to create NvMetamux \n")
    ...
    streammux_1 = Gst.ElementFactory.make("nvstreammux", "Stream-muxer-yolo")
    nvstreamdemux = Gst.ElementFactory.make("nvstreamdemux", "nvstreamdemux")

    streammux_2 = Gst.ElementFactory.make("nvstreammux", "Stream-muxer-face")
    nvstreamdemux_2 = Gst.ElementFactory.make("nvstreamdemux", "nvstreamdemux-2")

    # tee = Gst.ElementFactory.make("tee", "nvsink-tee")
    # pipeline.add(tee)

    pipeline.add(streammux_main)
    pipeline.add(streammux_1)
    pipeline.add(nvstreamdemux)
    pipeline.add(streammux_2)
    pipeline.add(nvstreamdemux_2)
    ...
    streammux_1.set_property('width', 1920)
    streammux_1.set_property('height', 1080)
    streammux_1.set_property('batch-size', number_sources)
    streammux_1.set_property('batched-push-timeout', 4)
    streammux_1.set_property('buffer-pool-size', 4)
    streammux_1.set_property('nvbuf-memory-type', memtype)

    streammux_2.set_property('width', 1920)
    streammux_2.set_property('height', 1080)
    streammux_2.set_property('batch-size', number_sources)
    streammux_2.set_property('batched-push-timeout', 10)
    streammux_2.set_property('buffer-pool-size', 4)
    streammux_2.set_property('nvbuf-memory-type', memtype)
    ...
    # queue_tee1=Gst.ElementFactory.make("queue","queuetee1")
    # queue_tee2=Gst.ElementFactory.make("queue","queuetee2")
    # pipeline.add(queue_tee1)
    # pipeline.add(queue_tee2)
    # sink_pad = queue_tee1.get_static_pad("sink")
    # tee_1_pad = tee.get_request_pad('src_%u')
    # tee_2_pad = tee.get_request_pad("src_%u")

    # tee_1_pad.link(sink_pad)

    # sink_pad = queue_tee2.get_static_pad("sink")
    # tee_2_pad.link(sink_pad)

    # queue_tee1.link(nvstreamdemux)
    # queue_tee2.link(nvstreamdemux_2)
    tee = []
    

    for i in range(number_sources):
        tmp_tee = Gst.ElementFactory.make("tee", "nvsink-tee-{}".format(i))
        pipeline.add(tmp_tee)
        ...
        # padname="sink_%u" %i
        # queuesinkpad = streammux_1.get_request_pad(padname)
        # if not queuesinkpad:
        #     sys.stderr.write("Unable to create queue sink pad \n")
        # demuxsrcpad_1.link(queuesinkpad)

    for i in range(8):
        # padname = "src_%u" % i
        # demuxsrcpad_1 = nvstreamdemux.get_request_pad(padname)
        # if not demuxsrcpad_1:
        #     sys.stderr.write("Unable to create demux src pad \n")
        tee_src_pad = tee[i].get_request_pad('src_%u')

        padname="sink_%u" %i
        queuesinkpad = streammux_1.get_request_pad(padname)
        if not queuesinkpad:
            sys.stderr.write("Unable to create queue sink pad \n")
        tee_src_pad.link(queuesinkpad)
    ...
    for i in range(number_sources):
        ...
        padname="sink_%u" %i
        queuesinkpad = streammux_2.get_request_pad(padname)
        if not queuesinkpad:
            sys.stderr.write("Unable to create queue sink pad \n")
        tee_src_pad.link(queuesinkpad)

    dead_sink = Gst.ElementFactory.make("fakesink", "nvvideo-renderer-deadsink")
    dead_sink.set_property("async", 0)
    dead_sink.set_property("qos",0)
    pipeline.add(dead_sink)

    streammux_2.link(pgie)
    pgie.link(dead_sink)

    streammux_1.link(pgie_2)
    pgie_2.link(tiler)
    # if nvdslogger:
