...
class ParkPTZ(InferBin):
    def __init__(self, pipeline, srcm, perf) -> None:
        self.app_name = 'parkptz'
        # self.bin = Gst.ElementFactory.make('bin', self.app_name)

        self.config_path = os.path.join(config.__path__[0], 'config_app_parkptz.txt')
        self.pgie_config_path = os.path.join(config.__path__[0], 'config_trt_yolov7.txt')
        self.sgie_config_path = os.path.join(config.__path__[0], 'config_trt_lpd.txt')
        self.tgie_config_path = os.path.join(config.__path__[0], 'config_trt_lpr.txt')
    ...
    def link_elements(self):
        ...
        # pgie_src_pad = self.main_bin_elements[1].get_static_pad("src")
